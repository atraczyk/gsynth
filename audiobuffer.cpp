...
void AudioBuffer::applyGain(double gain)
{
    ...
    if (g != gain) {
        DBGOUT("Normalizing %f to [-1.0, 1.0]", gain);
    }

    for (auto &channel : samples_) {
        for (auto &sample : channel) {
            sample = static_cast<AudioSample>(sample * g);
        }
    }
}
...
size_t AudioBuffer::mix(const AudioBuffer& other, bool up /* = true */)
{
    ...
    const unsigned chan_num = upmix ?
        samples_.size() :
        std::min(samples_.size(), other.samples_.size());
    for (size_t i = 0; i < chan_num; i++) {
        unsigned src_chan = upmix ?
            std::min<unsigned>(i, other.samples_.size() - 1) :
            i;
...
size_t AudioBuffer::copy(AudioBuffer& in, int sample_num /* = -1 */, size_t pos_in /* = 0 */, size_t pos_out /* = 0 */, bool up /* = true */)
{
    ...
    const size_t chan_num = upmix ?
        samples_.size() :
        std::min(in.samples_.size(), samples_.size());
    ...
    for (unsigned i = 0; i < chan_num; i++) {
        size_t src_chan = upmix ?
            std::min<size_t>(i, in.samples_.size() - 1U) :
            i;
